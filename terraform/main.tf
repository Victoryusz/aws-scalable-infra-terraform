...
provider "aws" {
  region = "us-east-1" // Correção: Faltava um hífen, antes estava "us-east1"
}
...
resource "aws_route_table" "public_rt" {
  ...
  # Cria rota que envia todo o trafego para a internet via IGW
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.gw.id # Correção: Antes estava "aws_internet_gateway.id", agora está correto
  }
...
resource "aws_eip_association" "eip_assoc" {
  instance_id   = aws_instance.n.id # Correção: Antes estava "aws_instance.web.id", agora está correto
...
resource "aws_autoscaling_group" "asg_web" {
  ...
  launch_template {
    ...
    version = "$Latest" # Correção: Antes estava "$Lastest", agora está correto
  }
...
#Comando para Conectar o Auto-Scaling-Group ao Load-Balancer!
resource "aws_autoscaling_attachment" "asg_alb_attachment" {
  autoscaling_group_name = aws_autoscaling_group.asg_web.name # Correção: Antes estava ".id", agora está correto com ".name"
